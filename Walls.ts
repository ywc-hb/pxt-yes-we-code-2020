class Walls {
    public wallNW: number[][];
    //public wallSW: number[][];
    //public wallNE: number[][];
    //public wallSE: number[][];

    constructor() {
        //this.mapTwo()
        this.mapOne();
        /*
        let walls = this.wallSW.concat(this.wallNW).concat(this.wallNE).concat(this.wallSE);
        let j = 0;
        while (j < walls.length) {
            LCD1IN8.DrawLine(walls[j][0], walls[j][1], walls[j][2], walls[j][3], 0, DOT_PIXEL.DOT_PIXEL_1, LINE_STYLE.LINE_SOLID);
            j++
        }*/

        for (let j = 0; j < this.wallNW.length; j++) {
            LCD1IN8.DrawLine(this.wallNW[j][0], this.wallNW[j][1], this.wallNW[j][2], this.wallNW[j][3], 0, DOT_PIXEL.DOT_PIXEL_1, LINE_STYLE.LINE_SOLID);
        }
        for (let j = 0; j < this.wallNW.length; j++) {
            LCD1IN8.DrawLine(160 - this.wallNW[j][0], this.wallNW[j][1], 160 - this.wallNW[j][2], this.wallNW[j][3], 25444, DOT_PIXEL.DOT_PIXEL_1, LINE_STYLE.LINE_SOLID);
        }
        for (let j = 0; j < this.wallNW.length; j++) {
            LCD1IN8.DrawLine(this.wallNW[j][0], 128 - this.wallNW[j][1], this.wallNW[j][2], 128 - this.wallNW[j][3], 50000, DOT_PIXEL.DOT_PIXEL_1, LINE_STYLE.LINE_SOLID);
        }
        for (let j = 0; j < this.wallNW.length; j++) {
            LCD1IN8.DrawLine(160 - this.wallNW[j][0], 128 - this.wallNW[j][1], 160 - this.wallNW[j][2], 128 - this.wallNW[j][3], 64512, DOT_PIXEL.DOT_PIXEL_1, LINE_STYLE.LINE_SOLID);
        }
        LCD1IN8.LCD_Display();
    }

    private mapOne(): void {
        //Fonction dessinant les murs de la map 1
        //Coordonnées des murs au format [[x_start, y_start, x_end, y_end], ...]
        this.wallNW = [[0, 0, 80, 0], [0, 0, 0, 64], [1, 50, 16, 50], [31, 34, 56, 34], [36, 59, 46, 59], [70, 64, 80, 64], [18, 12, 48, 12], [18, 1, 18, 12], [48, 1, 48, 12], [80, 1, 80, 25], [80, 45, 80, 64], [36, 59, 36, 64], [46, 59, 46, 64]];
        //this.wallSW = [[1, 128, 80, 128], [1, 64, 1, 128], [1, 78, 16, 78], [31, 94, 56, 94], [36, 69, 46, 69], [70, 64, 80, 64], [18, 116, 48, 116], [18, 116, 18, 128], [48, 116, 48, 128], [80, 64, 80, 85], [80, 103, 80, 128], [36, 64, 36, 69], [46, 64, 46, 69]];
        //this.wallNE = [[80, 1, 160, 1], [160, 1, 160, 64], [80, 1, 80, 25], [80, 45, 80, 64], [145, 50, 160, 50], [105, 34, 130, 34], [115, 59, 125, 59], [112, 12, 142, 12], [142, 0, 142, 12], [112, 0, 112, 12], [125, 59, 125, 64], [115, 59, 115, 64], [80, 64, 90, 64]];
        //this.wallSE = [[80, 128, 160, 128], [160, 64, 160, 128], [80, 64, 90, 64], [80, 64, 80, 85], [80, 103, 80, 128], [144, 78, 160, 78], [105, 94, 130, 94], [115, 69, 125, 69], [112, 116, 142, 116], [142, 116, 142, 128], [112, 116, 112, 128], [125, 64, 125, 69], [115, 64, 115, 69]];
    }
/*
    private mapTwo(): void {
        //Fonction dessinant les murs de la map 2
        //Coordonnées des murs au format [[x_start, y_start, x_end, y_end], ...]
        this.wallNW = [[60, 0, 60, 10], [1, 50, 11, 50], [70, 54, 80, 54], [60, 0, 60, 10], [55, 60, 55, 64], [20, 33, 38, 33], [11, 50, 11, 64], [70, 54, 70, 64], [60, 10, 80, 10], [55, 64, 70, 64], [38, 15, 38, 33], [35, 63, 35, 64], [80, 27, 80, 38]];
        this.wallSW = [[1, 78, 11, 78], [70, 74, 80, 74], [60, 118, 60, 128], [55, 64, 55, 68], [20, 93, 38, 93], [11, 64, 11, 78], [70, 64, 70, 74], [55, 64, 70, 64], [38, 93, 38, 111], [35, 64, 35, 65], [60, 118, 80, 118], [80, 91, 80, 105]];
        this.wallNE = [[80, 10, 100, 10], [80, 54, 90, 54], [90, 54, 90, 64], [150, 50, 160, 50], [100, 0, 100, 10], [105, 60, 105, 64], [120, 33, 138, 33], [150, 50, 150, 64], [80, 27, 80, 38], [90, 64, 105, 64], [120, 15, 120, 33], [125, 63, 125, 64]];
        this.wallSE = [[80, 74, 90, 74], [90, 64, 90, 74], [150, 78, 160, 78], [100, 118, 100, 128], [105, 64, 105, 68], [120, 93, 138, 93], [150, 64, 150, 78], [80, 118, 100, 118], [80, 91, 80, 105], [90, 64, 105, 64], [120, 93, 120, 111], [125, 64, 125, 65]];
    }*/
} 